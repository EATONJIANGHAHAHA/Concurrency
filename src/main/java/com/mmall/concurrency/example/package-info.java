...
死锁:有两个或两个以上的进程在职in修改过程中因争夺资源而造成互相等待的情况. 如果没有外力作用,他们都将无法推进下去,此时我们称这种情况为死锁.
并称这些互相等待的线程为死锁线程.
发生的必要条件:
1. 互斥条件, 进程对所分配到的资源进行了排他性的使用,即在一段时间内某资源只有一个线程使用,如果此时还有其他线程亲求资源,那么其他线程此时只能等待,
直到占有资源的线程结束并释放才可以继续执行.
2. 请求和保持条件, 线程已经保持了至少一个资源,但又提出了新的资源请求, 而该资源已被其他进程占有,此时请求线程阻塞,但又对自己已经获得的其他资源保持不放
3, 不剥夺条件, 进程已获得的资源在未使用完成的情况下不能被剥夺, 只能够自己释放
4, 环路等待条件, 发生死锁时, 一定有一个进程存在资源的环形链.
